...
# ====== FLOW (user_data keys) ======
# pending_step: None | "choose_slot" | "enter_kwh" | "await_photo" | "await_note" | "confirm"
# pending_slot: int
# pending_kwh: str
# pending_photo_id: str
# pending_note: str
...
# === Post-foto: pulsanti “Aggiungi nota” / “Conferma” ===
...
# === Nota testuale (facoltativa) — ORA invia subito la richiesta all’admin ===
async def on_note_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Subito invio come conferma con nota
    ...
    try:
        if ADMIN_ID:
            if qmessage:
                ...
                await update.message.bot.send_photo(chat_id=ADMIN_ID, photo=file_id, caption=caption, reply_markup=kb)
        user_text = f"Richiesta inviata all'amministratore (Slot {slot}, {fmt_kwh(kwh)} kWh)."
    except Exception:
        user_text = "Richiesta registrata, ma non è stato possibile avvisare l'amministratore."
...
async def on_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(q.from_user.id):
        await q.answer("Solo l'amministratore può gestire questa richiesta.", show_alert=True)
        return
    try:
    ...
    if status != "pending":
        await q.edit_message_caption(f"Richiesta #{rid} già {status}.")
        return
    kwh = qk(Decimal(str(kwh_f)))

    if action == "approve":
        add_balance_slot(u_id, slot, -kwh)  # saldo può andare in negativo
        ...
        try:
            await context.bot.send_message(
                chat_id=u_id,
                text=f"Ricarica #{rid} (Slot {slot}) rifiutata dall'amministratore."
            )
        except:
            pass
        await q.edit_message_caption(f"Richiesta #{rid} RIFIUTATA.")

# === Admin: accredita manualmente su slot ===
async def credita(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if slot not in SLOTS:
            raise ValueError
        kwh = qk(Decimal(context.args[2].replace(",", ".")))  # può essere negativo
...
# === Fallback generico (silenzioso) ===
async def fallback_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Ignora testi non pertinenti al flusso per non creare rumore.
    pass
...
def main():
    ...
    app.add_handler(CommandHandler("credita", credita))  # admin

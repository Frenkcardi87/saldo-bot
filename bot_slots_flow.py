...
async def on_message_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get("decl_await_photo"):
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ Aggiungi nota", callback_data="decl:note:add")],[InlineKeyboardButton("➡️ Procedi senza nota", callback_data="decl:note:skip")]])
...
async def render_pending_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    nav = [InlineKeyboardButton("⬅ Prev", callback_data="pend:prev"), InlineKeyboardButton("➡ Next", callback_data="pend:next")]
    row2 = [InlineKeyboardButton("📸 Foto", callback_data=f"pend:photo:{rid}"), InlineKeyboardButton("ℹ Info", callback_data=f"pend:info:{rid}")]
    row3 = [InlineKeyboardButton("✅ Approva", callback_data=f"pend:approve:{rid}"), InlineKeyboardButton("❌ Rifiuta", callback_data=f"pend:reject:{rid}")]
...
async def on_pending_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pend:reject:"):
        rid = int(data.split(":")[-1]); rec = DBI.get_recharge(rid); 
        if not rec: return await smart_reply(update, context, "Record non trovato.")
        DBI.set_recharge_status(rid, "rejected", update.effective_user.id); await smart_reply(update, context, f"Ricarica #{rid} *rifiutata* ❌")
...
async def render_wallet_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    nav = [InlineKeyboardButton("⬅ Prev", callback_data="wal:prev"), InlineKeyboardButton("➡ Next", callback_data="wal:next")]
    row2 = [InlineKeyboardButton("✅ Accetta", callback_data=f"wal:accept:{wid}"), InlineKeyboardButton("❌ Rifiuta", callback_data=f"wal:reject:{wid}")]
...
async def on_wallet_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wal:reject:"):
        wid = int(data.split(":")[-1]); DBI.set_wallet_status(wid, "rejected", update.effective_user.id); await smart_reply(update, context, f"Wallet #{wid} *rifiutato* ❌")
...
async def on_users_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("userdel:"):
        ...
        if choice == "yes": DBI.delete_user(uid); await smart_reply(update, context, f"Utente `{uid}` eliminato ✅")
